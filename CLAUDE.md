...
- **Adding a policy-group icon means updating two places.** `ProxyGroupIconCatalog` (macOS app, compiled in) *and* `assets/GroupIcons.json` (keyword → icon URL, fetched remotely by the Nest app). Nothing in the macOS app reads `GroupIcons.json`, so the coupling is invisible from the code. Keys are lowercase and named after the icon. Self-hosted icons live in `assets/icons/` and are referenced through jsDelivr pinned to a commit SHA; `.github/workflows/purge-jsdelivr.yml` purges the CDN on any `assets/**` push to `main`, so the JSON only takes effect for Nest once pushed.
