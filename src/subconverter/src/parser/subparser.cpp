...
void explodeClash(Node yamlnode, std::vector<Proxy> &nodes)
{
    ...
    std::string client_fingerprint, idle_session_check_interval, idle_session_timeout, min_idle_session; //anytls
    ...
    for(uint32_t i = 0; i < yamlnode[section].size(); i++)
    {
        ...
        case "anytls"_hash:
            ...
            anytlsConstruct(node, group, ps, server, port, password, sni, fingerprint, client_fingerprint, alpn, idle_session_check_interval, idle_session_timeout, min_idle_session, udp, tfo, scv, underlying_proxy);
            break;
